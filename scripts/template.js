...
const safeCopy = (from, to) => {
  if (!fs.existsSync(to)) {
    ...
    console.warn(`Refusing to overwrite ${to}`);
  }
};

safeCopy(path.join(yearDir, "template"), path.join(yearDir, `day-${date}.js`));
...
fs.appendFileSync(path.join(yearDir, "input", `day-${date}.txt`), "");
fs.appendFileSync(path.join(yearDir, "input", `day-${date}.test.txt`), "");
